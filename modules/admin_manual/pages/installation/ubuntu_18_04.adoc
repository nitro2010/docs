...
You can also enter `crontab -e -u www-data` and add the cron jobs you need.

[NOTE]
====
If you need to sync your users from an LDAP or Active Directory Server, add this additional xref:configuration/server/background_jobs_configuration.adoc[Cron job]. This will sync every 15 min. Not available users will be disabled and can be enabled later. Additionally you get a log file in `var/log/ldap-sync/user-sync.log` for debugging.
====
